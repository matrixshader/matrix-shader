...
| `Website/admin/emails/onboarding/welcome-operator.html` | `vX.Y.Z \| Windows 10/11` |
| `Website/303h4rdl1n3/emails/onboarding/welcome-operator.html` | `vX.Y.Z \| Windows 10/11` |

**CRITICAL**: All 8 files MUST be updated. Read each file first, then edit. If any file is missing or the pattern doesn't match, STOP and tell the user.

**TIP**: Use `grep -r` to scan for the OLD version string across the entire `Website/` directory to catch any other references that may have been added since this skill was written.
