...
function renderAll(data) {
  ...
  if (data.faq_questions) {
    renderFaqTab(data.faq_questions);
  }
  updateTimestamp();
}
...
function renderFaqTab(questions) {
  allFaqQuestions = questions || [];
  faqRendered = true;

  // Count by status
  let pending = 0, published = 0, dismissed = 0;
  for (const q of allFaqQuestions) {
    if (q.status === 'pending') pending++;
    else if (q.status === 'published') published++;
    else if (q.status === 'dismissed') dismissed++;
  }

  $('faq-pending-count').textContent = pending;
  $('faq-published-count').textContent = published;
  $('faq-dismissed-count').textContent = dismissed;
  $('faq-total-count').textContent = allFaqQuestions.length;
  $('faq-pending-badge').textContent = pending;

  renderFaqPending(allFaqQuestions.filter(q => q.status === 'pending'));
  renderFaqPublished(allFaqQuestions.filter(q => q.status === 'published'));
  renderFaqDismissed(allFaqQuestions.filter(q => q.status === 'dismissed'));
}

function renderFaqPending(items) {
  const el = $('faq-pending-table');
  if (!items.length) { el.innerHTML = '<div class="empty-state">No pending questions</div>'; return; }
  items.sort((a, b) => (b.submittedAt || '').localeCompare(a.submittedAt || ''));
  el.innerHTML = items.map(q => `<div class="faq-admin-item" id="faq-item-${esc(q.id)}">
    <div class="faq-admin-row">
      <div class="faq-admin-q">${esc(q.question)}</div>
      <div class="faq-admin-meta">
        <span>${esc(q.email || '')}</span> &middot;
        <span>${esc(q.category || 'general')}</span> &middot;
        <span>${fmtDate(q.submittedAt)}</span>
      </div>
    </div>
    <div class="faq-admin-actions">
      <button class="btn-sm faq-answer-btn" data-id="${esc(q.id)}">Answer</button>
      <button class="btn-sm faq-dismiss-btn" data-id="${esc(q.id)}">Dismiss</button>
      <button class="btn-sm faq-delete-btn" data-id="${esc(q.id)}" style="color:var(--red)">Delete</button>
    </div>
    <div class="faq-answer-form hidden" id="faq-form-${esc(q.id)}">
      <select class="faq-cat-select" id="faq-cat-${esc(q.id)}">
        ${['general','installation','usage','licensing','compatibility','troubleshooting'].map(c =>
          `<option value="${c}"${c === q.category ? ' selected' : ''}>${c}</option>`
        ).join('')}
      </select>
      <textarea class="faq-answer-input" id="faq-answer-${esc(q.id)}" placeholder="Write the answer..." rows="3">${esc(q.answer || '')}</textarea>
      <div style="display:flex;gap:8px;margin-top:6px">
        <button class="btn-sm faq-publish-btn" data-id="${esc(q.id)}" style="color:var(--green)">Publish</button>
        <button class="btn-sm faq-save-btn" data-id="${esc(q.id)}">Save draft</button>
        <button class="btn-sm faq-cancel-btn" data-id="${esc(q.id)}">Cancel</button>
      </div>
    </div>
  </div>`).join('');

  // Wire events
  el.querySelectorAll('.faq-answer-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const form = $(`faq-form-${btn.dataset.id}`);
  ...
  el.querySelectorAll('.faq-cancel-btn').forEach(btn => {
    btn.addEventListener('click', () => $(`faq-form-${btn.dataset.id}`).classList.add('hidden'));
  });

  el.querySelectorAll('.faq-dismiss-btn').forEach(btn => {
    btn.addEventListener('click', () => faqAction(btn.dataset.id, 'dismiss'));
  });

  el.querySelectorAll('.faq-delete-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      if (confirm('Permanently delete this question?')) faqDelete(btn.dataset.id);
    });
  });

  el.querySelectorAll('.faq-publish-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const answer = $(`faq-answer-${btn.dataset.id}`).value.trim();
      const category = $(`faq-cat-${btn.dataset.id}`).value;
      if (!answer) { alert('Answer is required to publish.'); return; }
      faqAction(btn.dataset.id, 'publish', { answer, category });
    });
  });

  el.querySelectorAll('.faq-save-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const answer = $(`faq-answer-${btn.dataset.id}`).value.trim();
      const category = $(`faq-cat-${btn.dataset.id}`).value;
      faqAction(btn.dataset.id, 'update', { answer, category });
    });
  });
}

function renderFaqPublished(items) {
  const el = $('faq-published-table');
  if (!items.length) { el.innerHTML = '<div class="empty-state">No published FAQs yet</div>'; return; }
  items.sort((a, b) => (b.publishedAt || '').localeCompare(a.publishedAt || ''));
  el.innerHTML = items.map(q => `<div class="faq-admin-item">
    <div class="faq-admin-row">
      <div class="faq-admin-q">${esc(q.question)}</div>
      <div class="faq-admin-a">${esc(q.answer || '')}</div>
      <div class="faq-admin-meta">
        <span>${esc(q.category || 'general')}</span> &middot;
        <span>Published ${fmtDate(q.publishedAt)}</span>
        ${q.answeredBy ? ` &middot; by ${esc(q.answeredBy)}` : ''}
      </div>
    </div>
    <div class="faq-admin-actions">
      <button class="btn-sm faq-edit-btn" data-id="${esc(q.id)}">Edit</button>
      <button class="btn-sm faq-reopen-btn" data-id="${esc(q.id)}">Unpublish</button>
    </div>
    <div class="faq-answer-form hidden" id="faq-edit-${esc(q.id)}">
      <select class="faq-cat-select" id="faq-ecat-${esc(q.id)}">
        ${['general','installation','usage','licensing','compatibility','troubleshooting'].map(c =>
          `<option value="${c}"${c === q.category ? ' selected' : ''}>${c}</option>`
        ).join('')}
      </select>
      <textarea class="faq-answer-input" id="faq-eanswer-${esc(q.id)}" rows="3">${esc(q.answer || '')}</textarea>
      <div style="display:flex;gap:8px;margin-top:6px">
        <button class="btn-sm faq-eupdate-btn" data-id="${esc(q.id)}" style="color:var(--green)">Save</button>
        <button class="btn-sm faq-ecancel-btn" data-id="${esc(q.id)}">Cancel</button>
      </div>
    </div>
  </div>`).join('');
  ...
  el.querySelectorAll('.faq-edit-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const form = $(`faq-edit-${btn.dataset.id}`);
      form.classList.toggle('hidden');
    });
  });

  el.querySelectorAll('.faq-ecancel-btn').forEach(btn => {
    btn.addEventListener('click', () => $(`faq-edit-${btn.dataset.id}`).classList.add('hidden'));
  });

  el.querySelectorAll('.faq-reopen-btn').forEach(btn => {
    btn.addEventListener('click', () => faqAction(btn.dataset.id, 'reopen'));
  });

  el.querySelectorAll('.faq-eupdate-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      const answer = $(`faq-eanswer-${btn.dataset.id}`).value.trim();
      const category = $(`faq-ecat-${btn.dataset.id}`).value;
      faqAction(btn.dataset.id, 'publish', { answer, category });
    });
  });
}

function renderFaqDismissed(items) {
  const el = $('faq-dismissed-table');
  if (!items.length) { el.innerHTML = '<div class="empty-state">No dismissed questions</div>'; return; }
  items.sort((a, b) => (b.dismissedAt || '').localeCompare(a.dismissedAt || ''));
  el.innerHTML = items.map(q => `<div class="faq-admin-item" style="opacity:0.6">
    <div class="faq-admin-row">
      <div class="faq-admin-q">${esc(q.question)}</div>
      <div class="faq-admin-meta">
        <span>${esc(q.email || '')}</span> &middot;
        <span>${fmtDate(q.dismissedAt)}</span>
      </div>
    </div>
    <div class="faq-admin-actions">
      <button class="btn-sm faq-reopen2-btn" data-id="${esc(q.id)}">Reopen</button>
      <button class="btn-sm faq-delete2-btn" data-id="${esc(q.id)}" style="color:var(--red)">Delete</button>
    </div>
  </div>`).join('');

  el.querySelectorAll('.faq-reopen2-btn').forEach(btn => {
    btn.addEventListener('click', () => faqAction(btn.dataset.id, 'reopen'));
  });

  el.querySelectorAll('.faq-delete2-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      if (confirm('Permanently delete?')) faqDelete(btn.dataset.id);
    });
  });
}

async function faqAction(id, action, extra = {}) {
  const headers = { 'Content-Type': 'application/json' };
  const session = getSession();
  const token = getToken();
  if (session) headers['X-Session'] = session;
  ...
  try {
    ...
    // Update local state
    const idx = allFaqQuestions.findIndex(q => q.id === id);
    if (idx >= 0) allFaqQuestions[idx] = updated;
    renderFaqTab(allFaqQuestions);
  } catch {
...
async function faqDelete(id) {
  ...
  try {
    const res = await fetch(`/api/faq?id=${id}`, { method: 'DELETE', headers });
    if (!res.ok) {
      const err = await res.json();
      alert(err.error || 'Failed');
      return;
    }
    allFaqQuestions = allFaqQuestions.filter(q => q.id !== id);
    renderFaqTab(allFaqQuestions);
  } catch {
    alert('Connection error');
  }
...
async function downloadBackup() {
  ...
  try {
    ...
    const a = document.createElement('a');
    a.href = URL.createObjectURL(blob);
    a.download = `matrixshader-backup-${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
  } catch {
