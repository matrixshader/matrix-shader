// MATRIX SHADER - SLOT 1 (GLSL / Kitty terminal)
...
// Kitty provides these uniforms
uniform float time;
uniform vec2 resolution;
uniform sampler2D image;  // terminal text content

// Bit-packed 5x7 Katakana glyphs - identical to HLSL version
...
vec3 DrawLayer(vec2 uv, float depth, float speed_mult, float brightness, float seed_shift) {
    ...
    vec2 grid_dims = resolution / baseCharSize;
...
void main() {
    ...
    tex.y = 1.0 - tex.y;  // flip Y - GLSL origin is bottom-left, terminals expect top-left
    ...
    vec4 text = texture(image, tex);
    gl_FragColor = text + vec4(totalRain * GLOW_STRENGTH, 0.0);
}
