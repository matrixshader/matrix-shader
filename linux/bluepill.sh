...
# Parse --preset flag (before --in-ghostty check so it can be forwarded)
PRESET_NAME=""
for arg in "$@"; do
    case "$arg" in
        --preset=*) PRESET_NAME="${arg#--preset=}" ;;
        --in-ghostty) ;; # handled below
    esac
done
...
SETUPEOF
    nohup "$GHOSTTY_BIN" --config-default-files=false --config-file="$setup_conf" -e "$SCRIPT_PATH" --in-ghostty ${PRESET_NAME:+--preset=$PRESET_NAME} >/dev/null 2>&1 &
    disown
    exit 0
fi
...
# --- Preset mode: launch single window from named preset ---
if [ -n "$PRESET_NAME" ]; then
    echo -e "${GREEN} Launching from preset '${PRESET_NAME}'...${RESET}"
    echo

    # Load preset params via Python
    preset_config=$(python3 -B -c "
import sys, json; sys.path.insert(0, '$PYMOD_DIR')
from preset_service import load_preset, list_presets
try:
    params = load_preset('$PRESET_NAME')
    print(json.dumps(params))
except FileNotFoundError:
    available = ', '.join(p['name'] for p in list_presets())
    print('ERROR:Preset not found. Available: ' + (available or 'none'), file=sys.stderr)
    sys.exit(1)
" 2>&1)
    exit_code=$?
    if [ $exit_code -ne 0 ]; then
        echo -e "${RED} $preset_config${RESET}"
        sleep 3
        exit 1
    fi
...
" 2>/dev/null)
    ...
    if [ -z "$slot" ]; then
        echo -e "${RED} All 8 shader slots are in use.${RESET}"
        sleep 3
        exit 1
    fi

    # Create shader with preset params
    shader_file=$(python3 -B -c "
import sys, json
sys.path.insert(0, '$PYMOD_DIR')
from shader_service import create_slot_shader, replace_define, atomic_write, PARAM_DEFAULTS
...
" 2>/dev/null)
    ...
    if [ -z "$shader_file" ] || [ ! -f "$shader_file" ]; then
        echo -e "${RED} Failed to create shader from preset.${RESET}"
        sleep 3
        exit 1
    fi

    # Derive fg color from preset RGB
    fg=$(python3 -c "
import json
c = json.loads('''$preset_config''')
r, g, b = c.get('RAIN_R', 0), c.get('RAIN_G', 1), c.get('RAIN_B', 0.3)
print('#%02x%02x%02x' % (int(r*255), int(g*255), int(b*255)))
" 2>/dev/null || echo "#00ff4d")

    # Read opacity from state.json (or default 85)
    opacity=$(python3 -c "
import sys; sys.path.insert(0, '$PYMOD_DIR')
from state_service import load_state
state = load_state()
...
" 2>/dev/null || echo 85)
    ...
    css_r=$(python3 -c "import json; c=json.loads('''$preset_config'''); print(int(c.get('RAIN_R',0)*255))" 2>/dev/null || echo 0)
    css_g=$(python3 -c "import json; c=json.loads('''$preset_config'''); print(int(c.get('RAIN_G',1)*255))" 2>/dev/null || echo 255)
    css_b=$(python3 -c "import json; c=json.loads('''$preset_config'''); print(int(c.get('RAIN_B',0.3)*255))" 2>/dev/null || echo 77)
    css_file="$MATRIX_TMP/ghostty-matrix-${slot}.css"
    cat > "$css_file" <<CSSEOF
@define-color headerbar_bg_color rgba(${css_r}, ${css_g}, ${css_b}, 0.15);
@define-color headerbar_backdrop_color rgba(${css_r}, ${css_g}, ${css_b}, 0.1);
@define-color headerbar_fg_color ${fg};
...
CSSEOF
    ...
    conf="$MATRIX_TMP/ghostty-matrix-${slot}.conf"
    cat > "$conf" <<EOF
custom-shader = ${shader_file}
background = #000000
foreground = ${fg}
...
EOF
    ...
    # Go transparent before launching
    go_transparent
    ...
    # Save preset config to state.json for this slot
    python3 -B -c "
import sys, json; sys.path.insert(0, '$PYMOD_DIR')
from state_service import load_state, save_state
config = json.loads('''$preset_config''')
state = load_state()
...
" 2>&1
    ...
    # Final status
    echo
    echo -e "${GREEN} Matrix-${slot}${RESET} ${DIM}launched from preset '${PRESET_NAME}'${RESET}"
    echo
    ...
    echo -e "${GOLD} \033]8;;https://buymeacoffee.com/IKnowKungFu\007buymeacoffee.com/IKnowKungFu\033]8;;\007${RESET}"
    echo
