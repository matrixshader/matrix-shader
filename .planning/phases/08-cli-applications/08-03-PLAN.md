...
must_haves:
  truths:
    - "wakeupneo.exe displays dramatic intro with typewriter effect"
    - "Arrow-key menu for Blue Pill / Red Pill choice"
    - "Blue Pill path: asks window count, lets user customize colors, launches windows"
  ...
  artifacts:
    - path: "MatrixShader/src/MatrixShader.Cli/WakeupNeo/Program.cs"
      provides: "Complete wakeupneo CLI with setup wizard"
      min_lines: 400
      exports: ["Main", "SetupWizard"]
  key_links:
    - from: "Program.cs"
      to: "CliBootstrap"
      via: "InitializeAsync and ArrowKeyMenu"
      pattern: "CliBootstrap\\."
    - from: "Program.cs"
      to: "IShaderService"
      via: "CreateShader for new windows"
      pattern: "CreateShader"
    - from: "Program.cs"
      to: "ITerminalSettingsService"
      via: "CreateMatrixProfiles"
...
<objective>
Rewrite wakeupneo.exe to match PowerShell matrix_setup.ps1 behavior: dramatic intro, Blue Pill / Red Pill choice, window count selection, color customization, window launch and positioning.

Purpose: wakeupneo.exe is the setup wizard for new users. It provides a cinematic Matrix-themed experience with the iconic Blue Pill / Red Pill choice. Blue Pill is simple setup, Red Pill additionally opens the control panel.

Output: Complete Program.cs for WakeupNeo CLI
</objective>
...
@matrix_setup.ps1 (PowerShell reference implementation)
...
<task type="auto">
  <name>Task 1: Rewrite WakeupNeo Program.cs with setup wizard</name>
  <files>MatrixShader/src/MatrixShader.Cli/WakeupNeo/Program.cs</files>
  <action>
Completely rewrite Program.cs to implement the setup wizard matching PowerShell behavior:
...
public class SetupWizard
{
    ...
    public async Task<int> RunAsync(bool morpheusMode, bool agentSmithMode)
    {
        ...
        foreach (var cfg in tabConfigs)
        {
            ...
            {
                _identityService.RegisterWindowHandle(newHandle, cfg.Slot);
        ...
        {
            var positions = _layoutService.CalculateLayout(allWindows, newState.LayoutConfig);
...
Key implementation details matching matrix_setup.ps1:
- Dramatic intro with typewriter effect ("Wake up, Neo...")
- Checks for previous session and offers restore
- Detects open Matrix windows to avoid slot collisions
- Asks for window count, lets user choose colors per tab
- Blue Pill / Red Pill choice using arrow-key menu
- Creates shaders and ensures profiles exist
- Launches windows via wt.exe
- Red Pill also launches control panel
- Agent Smith mode randomizes all windows
  </action>
  <verify>
Build succeeds:
```bash
cd MatrixShader && dotnet build src/MatrixShader.Cli/WakeupNeo/MatrixShader.Cli.WakeupNeo.csproj
```
  </verify>
  <done>WakeupNeo Program.cs rewritten with full setup wizard matching PowerShell behavior</done>
</task>

<task type="auto">
  <name>Task 2: Update WakeupNeo csproj to remove Spectre.Console</name>
  <files>MatrixShader/src/MatrixShader.Cli/WakeupNeo/MatrixShader.Cli.WakeupNeo.csproj</files>
  <action>
Update the project file to remove Spectre.Console reference (per CONTEXT.md: no third-party TUI libraries).

Check the csproj file and remove any PackageReference to Spectre.Console if present.
  </action>
  <verify>
...
```
  ...
  <done>WakeupNeo csproj updated, Spectre.Console reference removed if present</done>
</task>
...
1. `dotnet build MatrixShader/src/MatrixShader.Cli/WakeupNeo/MatrixShader.Cli.WakeupNeo.csproj` succeeds
2. Program shows "--help" when run with --help flag
3. Dramatic intro displays with typewriter effect
4. Arrow-key menu works for Blue Pill / Red Pill choice
5. Window count and color selection work
6. Windows launch via wt.exe
7. Red Pill path also launches control panel
8. --agent-smith triggers chaos mode
</verification>

<success_criteria>
- [ ] WakeupNeo Program.cs builds without Spectre.Console
- [ ] --help shows Matrix-themed help message
- [ ] --debug enables DiagnosticLogger
- [ ] --morpheus shows additional philosophical text
- [ ] --agent-smith randomizes all windows (chaos mode)
- [ ] Dramatic intro with "Wake up, Neo..." typewriter effect
- [ ] Previous session restore option offered
- [ ] Arrow-key menu for Blue Pill / Red Pill choice
- [ ] Window count selection (1-N based on available slots)
- [ ] Color preset selection with swatches
- [ ] Shaders created for configured tabs
- [ ] Windows launched via wt.exe
- [ ] Windows positioned after launch
- [ ] Red Pill path also opens control panel
- [ ] State saved for future restores
</success_criteria>

<output>
After completion, create `.planning/phases/08-cli-applications/08-03-SUMMARY.md`
</output>
